    ...
    function setupEditorClickHandlers() {
        ...
        elementor.on('preview:loaded', function() {
            ...
            previewDocument.addEventListener('click', function(e) {
                ...
                if (!isActive) {
                    ...
                    // For content-mode dropdown, clamp horizontally so it
                    // doesn't overflow the editor preview viewport.
                    clampContentDropdown(menuItem);
        ...
        function restoreActiveState() {
            ...
            Object.keys(state).forEach(function(widgetId) {
                ...
                // Re-clamp content-mode dropdown when state is restored.
                clampContentDropdown(menuItem);
        ...
        /**
         * For data-dropdown-width="content" items, clamp the dropdown so it
         * stays inside the editor preview viewport. The editor stylesheet
         * pins left: 0 !important, so we set our override with !important too.
         */
        function clampContentDropdown(menuItem) {
            if (!menuItem) return;
            if (menuItem.getAttribute('data-dropdown-width') !== 'content') return;

            var content = menuItem.querySelector('.db-mega-menu-content');
            if (!content) return;

            // Reset prior offset so the natural position is measured.
            // The editor stylesheet provides left: 0 !important as the base.
            content.style.removeProperty('left');

            var previewDoc = content.ownerDocument;
            var previewWin = previewDoc.defaultView || window;

            requestAnimationFrame(function () {
                var rect = content.getBoundingClientRect();
                var viewportWidth = previewWin.innerWidth || previewDoc.documentElement.clientWidth;
                var gutter = 16;
                var overflowRight = rect.right - (viewportWidth - gutter);
                var overflowLeft = gutter - rect.left;

                if (overflowRight > 0) {
                    content.style.setProperty('left', (-overflowRight) + 'px', 'important');
                } else if (overflowLeft > 0) {
                    content.style.setProperty('left', overflowLeft + 'px', 'important');
                }
            });
        }
    ...
    function updateDropdownStateFromModel() {
        try {
            ...
            menuItems.forEach(function(item, index) {
                ...
                // If this item is currently active, recalculate position
                // for both viewport and content modes.
                if (hasDropdown && domItem.classList.contains('e-active')) {
                    positionViewportDropdown(domItem);
                    clampContentDropdown(domItem);
                }
