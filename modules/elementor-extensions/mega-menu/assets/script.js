...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /**
     * For data-dropdown-width="content" items, ensure the dropdown stays
     * within the viewport. Default position is left-aligned to the item;
     * if that overflows the right edge, shift the dropdown left.
     */
    function applyContentClamp(item) {
        if (!item || item.getAttribute('data-dropdown-width') !== 'content') return;
        var content = item.querySelector('.db-mega-menu-content');
        if (!content) return;
    ...
    window.addEventListener('resize', function () {
        ...
        document.querySelectorAll('.db-mega-menu-item.e-active[data-dropdown-width="content"]').forEach(applyContentClamp);
    });
